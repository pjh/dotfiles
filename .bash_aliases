...
function reset-trackball {
	/homes/sys/pjh/bin/trackball.sh
}
...
function ssh-sampa {
	echo ssh pjh@sampa-gw.dyn.cs.washington.edu; ssh pjh@sampa-gw.dyn.cs.washington.edu
}
function ssh-sampa-X {
	echo ssh -X pjh@sampa-gw.dyn.cs.washington.edu; ssh -X \
	pjh@sampa-gw.dyn.cs.washington.edu
}
...
function my_cscope1 {
	...
	> cscope.files; time cscope -q -R -b -i cscope.files
}
...
# cvs:
function cvs-diff {
	cvs diff -c > cvs.diff; vi cvs.diff
}
function cvs-stat {
...
# svn:
function svn-diff {
	svn diff --diff-cmd /usr/bin/diff > svn.diff; vi svn.diff
}
function svn-diff-c {
	svn diff --diff-cmd /usr/bin/diff -x "-c" > svn.diff; vi svn.diff
}
...
function make-android-common {
	sudo update-java-alternatives -s java-1.5.0-sun; . build/envsetup.sh; \
...
function make-sdk-post {
	chmod 755 /homes/phornyac/android-sdk-current/; chmod 755 \
	/homes/phornyac/android-sdk-current/tools
...
function make-fastboot {
	fastboot flash boot out/target/product/passion/boot.img; fastboot flash \
	system out/target/product/passion/system.img; fastboot flash userdata \
...
function adb-reset {
	sudo /homes/phornyac/android-sdk-current/tools/adb kill-server; sudo \
	/homes/phornyac/android-sdk-current/tools/adb start-server; \
...
function logcat-out {
	logcat -c; logcat | tee logcat.out
}
function repo-diff {
	repo diff > repo.diff; vi repo.diff
}
function repo-status {
	repo status > repo.status; vi repo.status
}
function repo-branches {
	repo branches > repo.branches; vi repo.branches
}
function repo-forall {
	repo forall -c
}
function repo-diff-forall-unstaged {
	repo forall -c 'pwd; git diff' > repo.forall.unstaged.diff; vi \
	repo.forall.unstaged.diff
}
...
function repo-diff-forall-staged {
	repo forall -c 'pwd; git diff --cached' > repo.forall.staged.diff; vi \
	repo.forall.staged.diff
}
...
function repo-forall-branch {
	repo forall -c 'pwd; git branch' > repo.forall.branch; vi repo.forall.branch
}
function repo-forall-status {
	repo forall -c 'pwd; git status' > repo.forall.status; vi repo.forall.status
}
...
function django-src-rebuild {
	python setup.py install --prefix /homes/sys/pjh
}
